...
async function materializeAllCurrentSchemas(options = {}) {
    const schemaPaths = findSchemaPaths(options);

    return await Promise.all(schemaPaths
        .map(schemaPath => schemaPathToInfo(schemaPath, options))
        .filter(schemaInfo => schemaInfo.current)
        .map(schemaInfo => materializeSchema(schemaInfo.schema, options)));
}
